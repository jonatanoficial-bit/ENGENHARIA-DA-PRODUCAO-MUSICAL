...
if (firebaseReady && root) {
  ...
  authSdk.onAuthStateChanged(auth, async (user) => {
    ...
    } catch { root.innerHTML = '<p class="eyebrow">Meu perfil</p><h1>Não conseguimos carregar seu perfil agora.</h1><p class="lede">Seus dados continuam preservados. Tente novamente em alguns instantes.</p><button class="button button-secondary" type="button" data-profile-retry>Tentar novamente</button>'; root.querySelector('[data-profile-retry]')?.addEventListener('click', () => window.location.reload()); }
  });
}
