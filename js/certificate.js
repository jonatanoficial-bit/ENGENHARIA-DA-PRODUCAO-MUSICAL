...
import { calculateAcademicGrade, publishedScore, verifiedLessonCount } from './academic-model.js';
...
if (root && firebaseReady) {
  ...
  const score = (submissions, projects, activities) => {
    const tests = submissions.map(publishedScore).filter((value) => value !== null);
    ...
    const result = calculateAcademicGrade({ assessmentScores:tests, activityScores, continuousScore:publishedScore(continuous), finalScore:publishedScore(finalProject) });
  ...
  const render = (data) => {
    ...
    root.querySelector('[data-certificate-form]')?.addEventListener('submit', async (event) => {
      ...
      try {
        ...
        const verificationUrl = `https://producaomusical.org/pages/verificar-certificado.html?code=${encodeURIComponent(result.code)}`;
  ...
  authSdk.onAuthStateChanged(auth, async (user) => {
    ...
    try {
      ...
      const activityEntries = activities.docs.map(entry => ({id:entry.id,...entry.data()}));
      const completed = verifiedLessonCount(catalog.exists() ? catalog.data().completedLessons : [], activityEntries);
      const hours = settings.exists() && Number(settings.data().courseHours) > 0 ? Number(settings.data().courseHours) : 180;
      render({ user, name:student.exists() ? (student.data().name || student.data().email || user.displayName) : user.displayName, progress:Math.floor((completed / totalLessons) * 100), finalScore:score(entries,projects.docs.map((entry)=>({id:entry.id,...entry.data()})),activityEntries), hours });
