...
import { buildModuleReport, calculateAcademicGrade, formatGrade, TOTAL_LESSONS } from './academic-model.js';
...
if (firebaseReady && summaryNode) {
  try {
    ...
    const assessmentScores = assessments.filter((item) => item.status === 'graded' && Number.isFinite(Number(item.score))).map((item) => Number(item.score));
    const activityScores = activities.map((item) => Number.isFinite(Number(item.score)) ? Number(item.score) : null).filter((item) => item !== null);
    ...
    const grade = calculateAcademicGrade({ assessmentScores, activityScores, continuousScore:continuous?.score, finalScore:finalProject?.score });
