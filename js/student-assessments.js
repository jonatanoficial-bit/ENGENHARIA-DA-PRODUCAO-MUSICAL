...
if (firebaseReady && assessmentRoot) {
  ...
  authSdk.onAuthStateChanged(auth, async (user) => {
    if (!user) return;
    try {
      const [assessmentSnapshot, studentSnapshot] = await Promise.all([
      ...
      assessmentRoot.innerHTML = assessments.length ? `<p class="eyebrow">Provas publicadas pelo professor</p><h2>Avaliações integradas à sua trilha.</h2><div class="student-assessment-list">${assessments.map((assessment) => { const question = assessment.questions?.[0]; const result = assessment.submission?.status === 'graded' ? `<p class="form-feedback ${assessment.submission.passed ? 'form-feedback--ok' : 'form-feedback--error'}">Nota publicada: ${escapeHtml(assessment.submission.score)}% · ${assessment.submission.passed ? 'Aprovado' : 'Revisar o módulo'}</p>` : assessment.submission ? '<p class="form-feedback">Resposta enviada · aguardando correção.</p>' : ''; return question ? `<article class="student-assessment"><div><strong>${escapeHtml(assessment.title)}</strong><span>${escapeHtml(assessment.module)} · nota mínima ${escapeHtml(assessment.passScore)}%</span></div><form data-student-assessment="${assessment.id}"><fieldset><legend>${escapeHtml(question.prompt)}</legend>${question.options.map((option) => `<label><input type="radio" required name="answer" value="${escapeHtml(option)}"> ${escapeHtml(option)}</label>`).join('')}</fieldset><button class="button button--quiet" type="submit">Enviar resposta</button><p class="form-feedback"></p>${result}</form></article>` : ''; }).join('')}</div>` : '<p class="eyebrow">Provas publicadas pelo professor</p><h2>Avaliações integradas à sua trilha.</h2><p>Nenhuma avaliação foi publicada para sua turma ainda.</p>';
