  ...
  authSdk.onAuthStateChanged(auth, async (user) => {
    ...
    try {
      ...
      if (liveRoot) {
        ...
        liveRoot.innerHTML = `<div class="section-heading"><div><p class="eyebrow">Aulas ao vivo</p><h2>Próximos encontros da turma.</h2></div><span class="badge">Agenda integrada</span></div>${sessions.length ? `<div class="live-session-grid">${sessions.map((session) => `<article><strong>${safe(session.title)}</strong><span>${dateLabel(session.startsAt)}</span>${session.meetingUrl ? `<a class="button button--quiet" target="_blank" rel="noopener" href="${safe(session.meetingUrl)}">Entrar na aula ao vivo</a>` : '<small>O link será disponibilizado pela equipe antes do encontro.</small>'}</article>`).join('')}</div>` : '<div class="student-empty"><strong>Nenhuma aula ao vivo agendada.</strong><span>Continue pelas gravações disponíveis. O próximo encontro aparecerá aqui quando for publicado.</span></div>'}`;
      }
    } catch { if (root) { root.innerHTML = '<div class="student-empty"><strong>Não conseguimos carregar os projetos agora.</strong><span>Suas entregas continuam preservadas. Tente novamente em alguns instantes.</span></div><button class="button button-secondary" type="button" data-project-retry>Tentar novamente</button>'; root.querySelector('[data-project-retry]')?.addEventListener('click', () => window.location.reload()); } if (liveRoot) liveRoot.innerHTML = '<div class="student-empty"><strong>Não conseguimos consultar a agenda agora.</strong><span>Tente novamente em alguns instantes.</span></div>'; }
  });
}
